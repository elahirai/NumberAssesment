 ...
 public base64Output : any = '';
  ...
  submitData() {
    ...
    if (this.form.controls['patient'].value <= this.form.controls['maxCount'].value &&
      ...
      formData.append("maxNum", this.form.value.maxCount);
      formData.append("patientNum", this.form.value.patient);
      formData.append("doctNum", this.form.value.doctor);
      formData.append("message", this.base64Output);
      this.apiService.postData(formData)
        .subscribe(
          res => {
            ...
            this.form.reset();
            // debugger
            this.router.navigate(["/defualtComponent"],{ state: { res: res } })
  ...
  readTextFile(event: any) {
    ...
    if (file.name.slice(file.name.lastIndexOf('.') + 1, file.name.length) != 'txt') {
      ...
      this.getBase64(file).then(
        data => this.base64Output = data
      );
    this.convertFile(this.file);
    }
